...
RUN buildDeps='curl gcc g++ make autoconf libc-dev zlib1g-dev pkg-config' \
    ...
    && apt-get install --no-install-recommends --no-install-suggests -q -y \
        $buildDeps gnupg2 dirmngr wget apt-transport-https lsb-release ca-certificates \
    ...
    && apt-get upgrade -y \
    && apt-get install --no-install-recommends --no-install-suggests -q -y \
        locales git curl vim zip unzip cron procps \
        logrotate telnet net-tools iproute2 nmap ncat iputils-ping \
        apt-utils python-pip python-setuptools libmemcached-dev \
        libmemcached11 libmagickwand-dev nginx=${NGINX_VERSION} \
        php7.4-fpm php7.4-cli php7.4-bcmath php7.4-dev php7.4-common \
        php7.4-json php7.4-opcache php7.4-readline php7.4-mbstring \
        php7.4-curl php7.4-gd php7.4-mysql php7.4-zip php7.4-pgsql \
    ...
    && sed -i -e "s/pm.max_children = 5/pm.max_children = 64/g" ${fpm_conf} \
    && sed -i -e "s/pm.start_servers = 2/pm.start_servers = 8/g" ${fpm_conf} \
    && sed -i -e "s/pm.min_spare_servers = 1/pm.min_spare_servers = 4/g" ${fpm_conf} \
    && sed -i -e "s/pm.max_spare_servers = 3/pm.max_spare_servers = 32/g" ${fpm_conf} \
